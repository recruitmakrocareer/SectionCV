...
- โค้ดรองรับ LINE Login, ชื่อ/เบอร์ติดต่อ, อันดับผู้เล่นทั้งหมดแบ่งหน้าละ 10 คน, ป๊อปอัปเริ่มและจบเกม และกดผิด +5 วินาที
...
- กติกาใหม่ให้เวลาภาพละ 90 วินาที นับถอยหลังต่อเนื่องแม้สลับแอป กดผิดไม่เพิ่มเวลาที่เหลือ แต่เพิ่มเวลาจัดอันดับครั้งละ 5 วินาที
- เซิร์ฟเวอร์วัดเวลาจากรายการเริ่ม/พบคำตอบ/กดผิด/หมดเวลา ไม่รับคะแนนหรือเวลารวมที่ส่งมาจากผู้เล่น แต่ไม่ใช่ระบบป้องกันบอตสำหรับการแข่งขันที่มีรางวัลมูลค่าสูง
- เมื่อพบครบหรือหมดเวลา เก็บผลด่านนั้นและไปด่านถัดไปอัตโนมัติ ไม่นับช่วงเปลี่ยนและโหลดภาพ เมื่อเล่นจบทั้ง 3 ด่านบันทึกอันดับได้แม้หาได้ไม่ครบ 15 จุด
- เก็บผลดีที่สุดคนละหนึ่งอันดับ เรียงจำนวนจุดที่พบมากที่สุดก่อน ตามด้วยเวลารวมทั้ง 3 ด่านบวกโทษกดผิด ถ้ายังเท่ากันใช้จำนวนกดผิดน้อยกว่า เวลาที่บันทึกสำเร็จก่อน และรหัสรอบเพื่อให้ลำดับคงที่
- รอบกติกา 90 วินาทีใช้ `rulesVersion: 2` ในข้อมูล JSON ของ `runs.found` จึงไม่ต้องเพิ่มตารางหรือย้ายข้อมูล ผลกติกาเดิมยังอยู่ในประวัติและแยกจากตารางอันดับใหม่
